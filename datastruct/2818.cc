...
using Z = Modular<std::integral_constant<decay<decltype(P)>::type, P>>;
...
public:
  int maximumScore(vector<int>& nums, int k) {
    ...
    for (int i = 0; i < N; i++) {
      while (!stk.empty() && primes[nums[stk.back()]] < primes[nums[i]]) {
        R[stk.back()] = i;
        stk.pop_back();
      }
    ...
    for (auto &x : ord) {
      int64_t res = 1LL * (x - L[x]) * (R[x] - x);
      if (res >= k) {
        ans *= power(Z(nums[x]), k);
        break;
      } 

      ans *= power(Z(nums[x]), res);
      k -= res;
    }
...
#ifdef Reoden
int main() {
  std::vector a = {19,12,14,6,10,18};
  int x = 3;
  std::cout << Solution{}.maximumScore(a, x) << "\n";
